...
Usage:
    python charts/aggressiveness_ranking.py              # Current week
    python charts/aggressiveness_ranking.py --week 14    # Specific week
"""
...
def get_current_nfl_week() -> int:
    ...
    season_start = datetime.fromisoformat(week_one_start).replace(tzinfo=timezone.utc)
...
def calculate_aggressiveness_metrics(week: int, data_dir: str = "../out") -> pd.DataFrame:
    ...
    week_picks = week_picks.merge(
        consensus[['team', 'pick_percentage']],
        on='team',
        how='left'
    )
...
def create_aggressiveness_chart(metrics_df: pd.DataFrame, week: int, output_dir: str = "../out") -> str:
    """
    ...
    norm = plt.Normalize(vmin=metrics_df['total_aggressiveness'].min(),
                         vmax=metrics_df['total_aggressiveness'].max())
    ...
    ax1.set_title(f'Week {week} Aggressiveness Rankings - Risk×Conf Metric\n'
    ...
    for i, (_, row) in enumerate(metrics_df.iterrows()):
        ax1.text(row['total_aggressiveness'] + 0.3, i,
    ...
    ax2.set_title('Top 15 Boldest Individual Picks\n(Team(Confidence)@Field%)',
                  fontsize=12, fontweight='bold', pad=10)
    ax2.grid(axis='x', alpha=0.3)

    for i, (_, row) in enumerate(top_boldest.iterrows()):
        ax2.text(row['max_pick_aggressiveness'] + 0.1, i,
    ...
    scatter = ax3.scatter(metrics_df['avg_aggressiveness'],
    ...
    ax3.set_title('Strategy Profile: Consistent vs Bold\n(Bubble size = # Contrarian Picks)',
                  fontsize=12, fontweight='bold', pad=10)
    ax3.grid(alpha=0.3)
    ...
    # Add quadrant labels
    ax3.text(ax3.get_xlim()[1]*0.95, ax3.get_ylim()[1]*0.95,
             'Bold + Consistent', fontsize=9, ha='right', va='top',
             bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.3))
    ax3.text(ax3.get_xlim()[0]*1.05, ax3.get_ylim()[0]*1.05,
             'Conservative', fontsize=9, ha='left', va='bottom',
             bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.3))

    # ========== CHART 4: Distribution Histogram ==========
    ax4 = fig.add_subplot(gs[2, 0])

    ax4.hist(metrics_df['total_aggressiveness'],
             bins=12, edgecolor='black', linewidth=1.2, color='steelblue')
    ...
    ax4.set_title('Distribution of Total Aggressiveness',
                  fontsize=12, fontweight='bold', pad=10)
    ...
    ax4.axvline(median_val, color='orange', linestyle='--', linewidth=2, label=f'Median: {median_val:.1f}')
    ax4.legend(fontsize=10)

    # ========== CHART 5: Contrarian Analysis ==========
    ax5 = fig.add_subplot(gs[2, 1])

    contrarian_groups = metrics_df.groupby('num_contrarian').agg({
        'total_aggressiveness': 'mean',
        'player_name': 'count'
    }).reset_index()
    contrarian_groups.columns = ['num_contrarian', 'avg_aggressiveness', 'num_players']

    ax5.bar(contrarian_groups['num_contrarian'],
            contrarian_groups['avg_aggressiveness'],
            color='coral', edgecolor='black', linewidth=1.2)

    for i, row in contrarian_groups.iterrows():
        ax5.text(row['num_contrarian'], row['avg_aggressiveness'] + 0.3,
    ...
    ax5.set_title('Aggressiveness by Contrarian Pick Count\n(n = number of players)',
                  fontsize=12, fontweight='bold', pad=10)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate player aggressiveness rankings chart for NFL confidence pool',
    ...
    parser.add_argument(
        ...
        type=str,
        default='../out',
        help='Directory containing week result files (default: ../out)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='../out',
        help='Directory to save output chart (default: ../out)'
    )
    ...
    if not check_week_data_exists(week, args.data_dir):
    ...
    try:
        metrics_df = calculate_aggressiveness_metrics(week, args.data_dir)
    ...
    print(f"Formula: Aggressiveness = (1 - Field%) × Confidence")
    print(f"{'='*120}\n")

    print(f"{'Rank':<6} {'Player':<25} {'Total':>8} {'Avg':>8} {'MaxPick':>8} {'Boldest Pick':<20} {'Points':>8} {'Wins':>6}")
    print(f"{'-'*120}")
    ...
    for i, (_, row) in enumerate(metrics_df.iterrows(), 1):
        ...
        print(f"{i:<6} {row['player_name']:<25} "
    ...
    # Save metrics CSV
    csv_path = os.path.join(args.output_dir, f'week_{week}_aggressiveness_metrics.csv')
    metrics_df.to_csv(csv_path, index=False)
    print(f"✓ Metrics saved to: {csv_path}")

    # Create visualization
    print(f"\nGenerating visualization...")
    chart_path = create_aggressiveness_chart(metrics_df, week, args.output_dir)
    print(f"✓ Chart saved to: {chart_path}")

    # Summary
    print(f"\n{'='*120}")
    print(f"SUMMARY:")
    print(f"  Most Aggressive: {metrics_df.iloc[0]['player_name']} ({metrics_df.iloc[0]['total_aggressiveness']:.2f})")
